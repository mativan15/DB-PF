...
function displayResults(flights) {
    ...
    const questionParagraph = document.createElement('p');
    questionParagraph.className = 'parrafo';
    questionParagraph.textContent = flights[0]?.Precio || "No hay vuelos disponibles"; // Mostrar precio del primer vuelo o mensaje si está vacío
    ...
    flights.forEach(flight => {
        ...
        flightDetails.innerHTML = `
            <div class="flight-segment">
                ...
                <p class="route">AQP <span class="arrow">→</span> ${flights[0].Ciudad_Destino.toUpperCase()}</p>
                <p class="airline">${flights[0].Num_Vuelo}</p>
                <p class="time">${flights[0].Fecha_Salida} <span class="layover">${flights[0].Pais_Destino}</span></p>
                <p class="duration">${flights[0].Precio}</p>
            </div>
            <div>
                <p class="route">$${flights[0].Precio}</p>
            </div>
        `;
